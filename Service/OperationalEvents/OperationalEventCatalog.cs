...
public sealed class OperationalEventCatalog
{
    ...
    public OperationalEventCatalog()
    {
        ...
        [
            ...
            Define("CRANE_MAGNET_OFF_FAILED", "退磁命令结果未知", OperationalEventSeverity.Critical, OperationalEventCategory.Magnet,
                "现场确认磁铁实际状态、工件是否已释放、目标位置和Z高度。", "禁止把退磁异常解释为确定仍吸附或确定已释放；禁止在退磁结果未知时移动工件所有权或通知已放料。"),
            ...
            Define("CRANE_X11_UNEXPECTED_WORKPIECE", "X11发现非预期残留工件", OperationalEventSeverity.Critical, OperationalEventCategory.Safety,
                "保持原流程拒绝取料，现场确认磁铁上的真实工件身份、来源、当前设备位置和Z高度。", "禁止把残留工件冒充为本次计划工件；禁止清除身份后直接重新取料。"),
