...
public sealed class Line2FrontFlowEngine : IDisposable
{
    ...
    private readonly MarkerShareMonitor _markerMonitor = new("2号线打号机", MarkerSharePath);
    ...
    private void PauseForkBoringHandshake(string reason, Exception ex)
    {
        ...
        _paused = true;
        Console.WriteLine($"[Line2Front] [货叉] {reason}: {ex.Message} → 保持阶段={_forkPhase}, 引擎已暂停, 请人工确认货叉/双头镗/工件位置");
    }
    ...
    private async Task ProcessCraneFrontAsync(WorkpieceCache wp, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            Console.WriteLine("[Line2Front] [前天车] ②.5 货叉回待机 + 前天车Z升安全 并发执行");
            // 前天车所有绝对Z目标都叠加天车自身偏移；当前偏移通常为0, 但不能省略。
