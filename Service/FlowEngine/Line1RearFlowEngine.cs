    ...
    private async Task DoUnload(SkewCtx bed, CancellationToken ct)
    {
        ...
        try
        {
            ...
            if (sharedLocked)
            {
                ...
                {
                    _paused = true;
                    Console.WriteLine("│ [下料] ⚠ ST108共享区退避失败, 未释放共享区锁; 引擎已暂停, 请人工确认后天车位置");
                    throw new InvalidOperationException("ST108下料完成后共享区退避失败, 已暂停, 未释放共享区锁");
                }
            }
        ...
        catch (Exception ex)
        {
            ...
            if (holdingWorkpiece && !placedToDestination)
            {
                ...
                OnRearCraneSafetyAlarm?.Invoke($"1号线后天车从{bed.Code}下料时发生异常，X11已确认工件在天车上，但目标位放料或通知尚未完成。引擎已暂停，请人工确认天车和工件位置。异常：{ex.Message}");
            }
            else if (magnetOn)
            {
        ...
        finally
        {
            if (sharedLocked)
            {
                ...
                else
                {
                    ...
                    OnRearCraneSafetyAlarm?.Invoke($"1号线后天车在{bed.Code}下料异常收尾时无法退出共享区，共享区锁未释放。引擎已暂停，请人工确认天车位置。");
                }
                }
            }
            operation.TryRelease("RearCrane", _craneRearLock);
