    ...
    private async Task ProcessCraneFrontAsync(WorkpieceCache wp, CancellationToken ct = default)
    {
        ...
        catch (Exception ex)
        {
            ...
            if (ex is TransferRackCacheException rackEx)
            {
                ...
                Console.WriteLine($"[Line1Front] [前天车] 货叉—双头镗阶段保持={_forkPhase}，相关门闩未清除；中转架锁/共享区锁已按finally释放，前天车锁将在调度finally释放");
                OnSafetyAlarm?.Invoke($"1号线前天车处理{wp.IdentityText}时，工件已放到{rackEx.RackStation}，但后端缓存未确认。引擎已暂停，货叉—双头镗阶段保持={_forkPhase}、相关门闩未清除。中转架锁和共享区锁已释放，前天车锁将在任务退出时释放。请人工补缓存，确认旧工件和前天车处于安全位置后再恢复。");
            }
            else if (magnetOn)
            {
                ...
                Console.WriteLine($"[Line1Front] [前天车] 货叉—双头镗阶段保持={_forkPhase}，相关门闩未清除；中转架锁/共享区锁已按finally释放，前天车锁将在调度finally释放");
                OnSafetyAlarm?.Invoke($"1号线前天车处理{wp.IdentityText}时流程中断，工件可能在天车、打号机或后续位置。引擎已暂停，货叉—双头镗阶段保持={_forkPhase}、相关门闩未清除。中转架锁和共享区锁已释放，前天车锁将在任务退出时释放。请人工退磁/处理旧工件并将前天车升到安全位置后再恢复。异常：{ex.Message}");
            }
            else
            {
