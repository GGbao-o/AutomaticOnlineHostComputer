...
///   _transferRackLock: 串行化中转架操作
///   前天车锁顺序: _craneFrontLock → SharedAreaLock → _transferRackLock；后端靠共享区斜床也按 SharedAreaLock → _transferRackLock。
///   _manipulatorLock 独立，不和共享区/中转架锁嵌套。
///
/// 【下压信号处理 — 下压不是异常】
...
public sealed class Line1FrontFlowEngine : IDisposable
{
    ...
    /// <summary>货叉-双头镗交互状态机 — 新 Modbus 握手；R6102/R6104/R6108 只写1不清零。</summary>
    ...
    public void Pause() { _paused = true; Console.WriteLine("[Line1Front] ⏸ 暂停"); }
                ...
                if (_forkSvc != null && await _forkDispatchLock.WaitAsync(0, ct))
                {
                    try
                    {
                        ...
                        if (fs.HasPlate && fs.AtStandbyPos && _currentWp != null && _forkPhase == ForkBoringPhase.Idle)
                        {
                            ...
                            {
                                if (_cycleCount % 10 == 1)
                                    Console.WriteLine("[Line1Front] [货叉分派] ⚠ 机械手1未回安全Y位(可能在叉区),等待...");
                            }
                            //机械手在安全位置
                            else
                            {
                        ...
                        {
                            // 诊断: 条件不满足时打印各字段
                            Console.WriteLine($"[Line1Front] [货叉分派] 条件不满足 HasPlate={fs.HasPlate} Standby={fs.AtStandbyPos} 在途={_currentWp != null} 阶段={_forkPhase}");
                        }
                    }
                    catch (Exception ex)
                    {
                            ...
                            case ForkBoringPhase.Load_WriteParams:
                                if (_boringSvc != null && _boringSvc.IsConnected && !_forkHandshakeInProgress)
                                {
                                    ...
                                    _ = Task.Run(async () =>
                                    {
                                        try
                                        {
                                            Console.WriteLine("[Line1Front] [货叉] ① 等R6101=1请求数据...");
                                            ...
                                            Console.WriteLine(
                                                $"[Line1Front] [货叉] ② 写Modbus参数 R2041={wp.Length} R2043={boringD}*100 R2044={wp.LeftPlugThickness}*100 R2045={wp.RightPlugThickness}*100 R2047={wp.BoreType}*100");
                                            ...
                                            Console.WriteLine("[Line1Front] [货叉] ③ R6102=1 数据下发完成 ✓ → 等R6103=1...");
                                            await WaitForBoringSignalAsync(
                            ...
                            case ForkBoringPhase.Unload_GoPos3:
                                if (!_forkHandshakeInProgress)
                                {
                                    ...
                                    else if (fs.HasPlate && !_boringUnloadDoneSent)
                                    {
                                        ...
                                        Console.WriteLine($"[Line1Front] [货叉] R6108=1 下料完成 ✓ → 工件入天车队列 {_currentWp?.IdentityText}");
    ...
    private async Task ProcessManipulatorPickupAsync(WorkpieceCache wp, CancellationToken ct)
    {
        ...
        try
        {
            ...
            for (int retry = 0; retry < 3; retry++)
            {
                ...
                catch (PressureStopException) { Console.WriteLine($"[Line1Front] [机械手1]   ⚠ Z升下压误触→清除→重试{retry+1}"); await RecoverFromPressureStopAsync(ct); }
            }
        ...
        catch (Exception ex)
        {
            ...
            else if (placedOnFork)
            {
                ...
                Console.WriteLine("[Line1Front] [机械手1] 工件已安全放在货叉上, 保持 _currentWp 等 ForkBoringPhase 接管");
            }
            else
            {
    ...
    private async Task ProcessCraneFrontAsync(WorkpieceCache wp, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            // ① 等机械手1回安全位
            Console.WriteLine($"[Line1Front] [前天车] ① 等机械手1安全位 Y={_cfg.SkewBed.Manipulator1SafeY}mm...");
            ...
            // ②.5 X11已经确认吸住: 货叉只回待机位, 前天车只升Z安全高度, 两个动作互不进入对方碰撞区域。
            //      并发等待两边都完成；任一动作异常仍会抛出, 由外层catch按"工件在天车上"处理。
            Console.WriteLine("[Line1Front] [前天车] ②.5 货叉回待机 + 前天车Z升安全 并发执行");
            // 前天车所有绝对Z目标都叠加天车自身偏移；当前偏移通常为0, 但不能省略。  会安全位置z轴就不需要加偏移了
            ...
            await Task.WhenAll(forkStandbyTask, zSafeTask);

            // ③ 等中转架空位 → 获取共享区锁 → 获取中转架锁 → 打号机 → 放中转架
            //    预检: 锁外先等中转架空位, 避免抢到锁后发现全满(后天车无法取料→死锁)。
            //    安全: 共享区锁后立即拿中转架锁, 防止前天车打号期间后天车进入中转架1/2取料造成碰撞。
            ...
            Console.WriteLine("[Line1Front] [前天车] ③ 获取共享区锁...");
            while (!await _safety.SharedAreaLock.WaitAsync(TimeSpan.FromSeconds(1), ct))
            {
                if (_cycleCount % 10 == 1)
                    Console.WriteLine("[Line1Front] [前天车] 等共享区锁...");
            }
            Console.WriteLine("[Line1Front] [前天车] 共享区锁已获取 ✓");
            bool transferRackLocked = false;
            try
            {
                Console.WriteLine("[Line1Front] [前天车] ③.5 获取中转架锁(打号前预占)...");
                while (!transferRackLocked)
                {
                    transferRackLocked = await _transferRackLock.WaitAsync(500, ct);
                    if (!transferRackLocked) Console.WriteLine("[Line1Front] [前天车] 等中转架锁(后端取料占用中)...");
                }
                Console.WriteLine("[Line1Front] [前天车] 中转架锁已获取 ✓");
                ...
                Console.WriteLine($"[Line1Front] [前天车] 中转架放料完成, 中转架锁已释放 ✓ {wp.IdentityText}");
            ...
            finally
            {
                if (transferRackLocked)
                {
                    ...
                    Console.WriteLine("[Line1Front] [前天车] 异常兜底: 中转架锁已释放");
                }
                //共享区锁释放
                _safety.SharedAreaLock.Release();
                Console.WriteLine("[Line1Front] [前天车] 共享区锁已释放 ✓");
            }
        }
        catch (Exception ex)
        {
    ...
    private async Task MarkingHandshakeAsync(CraneService crane, WorkpieceCache wp, int safeZ, CancellationToken ct)
    {
        ...
        // Z升+写A.txt 并发启动
        int markerZUp = markerZ - 300 + _craneOffsetZ; // 上升目标Z = 取料位-200+天车偏移
    ...
    /// <summary>
    /// 中转架放料:
    ///   获取或使用外层已预占的 _transferRackLock → 读M811~M813 → 选第一个空位(ST105→ST101→ST106)
    ...
    private async Task PlaceOnTransferRackAsync(CraneService crane, WorkpieceCache wp, int safeZ, CancellationToken ct, bool transferRackLockAlreadyHeld = false)
    {
        ...
        {
            Console.WriteLine("[Line1Front] [中转架] ⑤ 使用前天车打号前已持有的_transferRackLock");
        }
        else
        {
            Console.WriteLine("[Line1Front] [中转架] ⑤ 获取_transferRackLock...");
            ...
            while (!gotTransferRack)
            {
                ...
                if (!gotTransferRack) Console.WriteLine("[Line1Front] [中转架] 后端取料占用中, 等待...");
            }
        }
        ...
        try
        {
            ...
            // ── 按优先级选空位 (M811→ST105, M812→ST101, M813→ST106) ──  如果没有板了 现在天车就会停在打号机正上方
            ...
            else throw new InvalidOperationException("1号线3个中转架全满，无法放料！");//抛出异常  直接会释放中转架锁
        ...
        finally
        {
            ...
            if (gotTransferRack && !transferRackLockAlreadyHeld)
            {
                ...
                Console.WriteLine("[Line1Front] [中转架] _transferRackLock 已释放");
            }
        }
    }
    ...
    private async Task WaitForBoringSignalAsync(Func<Task<bool>> check, string name, CancellationToken ct)
    {
        ...
        int pollMs = _cfg.Grinding.SignalPollIntervalMs; // 紧轮询(默认50ms), 不等主循环500ms
    ...
    private async Task WaitForManipulatorSafeAsync(CancellationToken ct)
    {
        ...
        Console.WriteLine($"[Line1Front] 机械手1 安全检查: 目标Y={safeY} 超时={_cfg.Grinding.HandshakeTimeoutMs / 1000}s");
        while (DateTime.UtcNow < dl)
        {
            ...
            try
            {
                ...
                if (Math.Abs(s.YPos - safeY) <= 10) { Console.WriteLine($"[Line1Front] 机械手1 Y={s.YPos} 在安全位 ✓"); return; }
                Console.WriteLine($"[Line1Front] 机械手1 Y={s.YPos} 安全Y={safeY} 等待中...");
            }
            catch (OperationCanceledException) when (ct.IsCancellationRequested) { throw; }
