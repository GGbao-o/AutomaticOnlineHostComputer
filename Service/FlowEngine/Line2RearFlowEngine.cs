...
public sealed class Line2RearFlowEngine : IDisposable
{
                        ...
                        if (await _transferRackLock.WaitAsync(0, ct))
                        {
                            ...
                            try
                            {
                                ...
                                else
                                {
                                    ...
                                    // ST606紧邻中转架区, 先释放中转架锁避免与SharedAreaLock反向持锁死锁。
                                    // DoLoad内部会按 SharedAreaLock→_transferRackLock 顺序重新获取。
                                    if (pair.Bed.Code == "ST606")
                                    {
                                        _transferRackLock.Release();
                                        gotTransferRack = false;
                                        Console.WriteLine("│   ST606: 释放中转架锁(DoLoad内部重新按顺序获取)");
                                    }

                                    pair.Bed.St = SkewState.Loading; triggered = true;
                                    gotTransferRack = false; // 锁所有权交给DoLoad; ST606会在DoLoad内重新获取
                                    _ = DoLoad(pair.Bed, pair.RackCode, ct); // 异步执行, 不阻塞主循环
                                }
                            }
                            catch
                            {
                                // 只释放本轮仍由主循环持有的中转架锁, 避免ST606已释放/已转交后误放别人的锁。
                                if (gotTransferRack) _transferRackLock.Release();
                            }
                        }
                        else { Console.WriteLine("│   中转架锁被前端占用,下轮重试"); }
    ...
    private List<SkewCtx> GetReadyBedsByPriority()
    {
        ...
        // ST606靠近前天车共享区。共享区忙时临时优先其它斜床,
        // 减少后天车拿着后天车锁等待SharedAreaLock; 无其它选择时仍允许ST606兜底。
        if (_safety.SharedAreaLock.CurrentCount == 0)
        {
            ...
            {
                if (result.Count == 0)
                    Console.WriteLine("│   共享区忙且无其它斜床可用, 仍允许ST606等待共享区");
                result.Add(_beds[0]);
            }
            return result;
        }
    ...
    private static string ActiveOperationText(SkewCtx bed)
        => bed.ActiveOperation == null
            ? "无"
            : $"{bed.ActiveOperation.Name} v{bed.ActiveOperation.Version}(后天车锁={YN(bed.ActiveOperation.IsHeld("RearCrane"))} 中转架锁={YN(bed.ActiveOperation.IsHeld("TransferRack"))} 共享区锁={YN(bed.ActiveOperation.IsHeld("SharedArea"))} 分流M818={YN(bed.ActiveOperation.IsHeld("M818"))} 分流M821={YN(bed.ActiveOperation.IsHeld("M821"))})";
    ...
    {
        bool transferLocked = bed.Code != "ST606"; // 非ST606由主循环已持有; ST606会先拿共享区再重拿中转架锁
        WorkpieceCache wp;
        long rackArrivalSeq;
        lock (_wpLock) {
        ...
        try
        {
            // 仅ST606紧邻中转架区, 取料时需锁共享区防前车同时进入碰撞
            if (bed.Code == "ST606")
            {
                // ST606: 按 SharedAreaLock→_transferRackLock 顺序获取(主循环已先行释放中转架锁)
                //   避免与前台天车 SharedAreaLock→_transferRackLock 反向持锁导致死锁
                Console.WriteLine("│ [上料] 获取共享区锁(ST606)...");
                await operation.AcquireAsync("SharedArea", _safety.SharedAreaLock, ct);
                sharedLocked = true;
                Console.WriteLine("│ 共享区锁已获取(ST606) ✓");
        ...
        finally
        {
            ...
            if (sharedLocked)
            {
                ...
                else
                {
                    ...
                    {
                        _paused = true;
                        pendingSafetyAlarm = $"2号线后天车在{bed.Code}上料异常收尾时未完成共享区X退避。引擎已暂停，请立即人工确认天车位置";
                    }
                }
            }
    ...
    private async Task DoUnload(SkewCtx bed, CancellationToken ct)
    {
        ...
        try
        {
            ...
            // 仅ST606紧邻中转架区, 下料取料时需锁共享区防前车同时进入碰撞
            if (bed.Code == "ST606")
            {
            ...
            if (sharedLocked)
            {
                ...
                if (await TryRetreatSharedAreaBeforeReleaseAsync(cr, bed.Code, "下料", ct))
                {
                    ...
                    sharedLocked = false;
                    Console.WriteLine("│ [下料] ST606已退离共享区并写下料完成, 提前释放共享区锁");
                }
                else
                {
                    _paused = true;
    ...
    /// <summary>
    /// 共享区锁兜底释放前的安全退避。
    /// finally里仍持有SharedAreaLock, 说明流程没有走到正常提前释放点, 后天车可能停在ST606上方。
