...
例如 ST112 基准 `AbsY=-779`，动态下料位的 `yOff=170`。Y 显示坐标从基准位置向负方向移动 170 mm，而 D5020 与显示 Y 反向，所以动态目标应为 `-779 + 170 = -609`。日志中的当前 `AbsY=-609` 实际已经到位，却被固定 `-779` 拦截。

## 范围

- 覆盖 1、2 号线后天车的斜床下料取料前（Line1Rear、Line2Rear）。
- 保留当前 Y 反向显示移动公式、共同稳定确认、最多两次微调、单次 XY 联动、超限禁止 Z 下降。
- 固定 Y 工位继续使用配置中的基准绝对值，不改变前天车、研磨天车、后天车中转架或斜床上料前行为。
- 不改异常监控/弹窗策略。

## 设计

`XAbsFineTuneHelper.VerifyAndFineTuneAsync` 增加可选参数 `yTargetAbsOffsetMm`，默认 `0`。

```text
X 目标Abs = 配置基准X
Y 目标Abs = 配置基准Y + yTargetAbsOffsetMm
```

两条后天车的斜床下料调用传入当前工件的 `yOff`。因当前下料显示目标是 `基准Y - yOff`，且 D5020 与显示 Y 反向，Y 绝对值目标必须加 `yOff`。
